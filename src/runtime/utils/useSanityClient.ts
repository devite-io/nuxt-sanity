...
export default function useSanityClient(type?: 'minimal' | 'default'): SanityClient {
  ...
  const sanityConfig = (import.meta.client ? $config.public.sanity : defu($config.sanity, $config.public.sanity || {})) as ClientConfig & ModuleOptions

  const visualEditingEnabled = sanityConfig.visualEditing
    && (!sanityConfig.visualEditing.previewMode || (import.meta.client && useSanityVisualEditingState().enabled))
  const clientType = visualEditingEnabled ? 'default' : (type || (sanityConfig.minimalClient ? 'minimal' : 'default'))
  ...
  if (visualEditingEnabled)
    (client as DefaultSanityClient).createQueryStore()
